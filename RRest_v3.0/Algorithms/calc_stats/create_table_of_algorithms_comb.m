function create_table_of_algorithms_comb(up)
...
for group_no = 1 : length(groups)
    ...
    %% Make combined table of results
    comb_results_table = create_table_results(alg_names, rel_data, up, group, 'comb');
        
    %% Save to file
    save_name = [up.paths.filenames.comb_results_table, '_', group];
    savepath = [up.paths.tables_save_folder, save_name];
    xlswrite(savepath, comb_results_table)
