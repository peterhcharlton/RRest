...
rel_els = find_rel_subjs(up, 'group');
...
for subj = subjs
    ...
    for s = 1 : length(alg_names.old_names)
        ...
        rel_ekg_alg = up.paramSet.ekg_sigs{1};
        eval(['data.sqi(start_el:counter_no,1) = sqi.' rel_ekg_alg '.v;']);
        eval(['data.hr(start_el:counter_no,1) = sqi.' rel_ekg_alg '.hr;']);
        eval(['data.hr_rr(start_el:counter_no,1) = sqi.' rel_ekg_alg '.hr./rr_ref.v;']);
    end
...
for alg_no = 1 : length(old_names)
    ...
    rel_name = strrep(rel_name, 'SPA', 'Fm2');
    rel_name = strrep(rel_name, 'PMC', 'Fm3');
    rel_name = strrep(rel_name, 'PRC', 'Fm4');
    
    % Find signal name
    for sig_type = [up.paramSet.ekg_sigs; up.paramSet.ppg_sigs; up.paramSet.scg_sigs]'
        if ~isempty(strfind(rel_name, sig_type{1,1}))
            alg_names.sigs{alg_no} = sig_type{1,1};
        end
    end
    
    % Tidy up
    for sig_type = [up.paramSet.ekg_sigs; up.paramSet.ppg_sigs; up.paramSet.scg_sigs]'
        rel_name = strrep(rel_name, [sig_type{1,1} '_'], '');
    end
...
%% Find out which subgroup each subject is in
if sum(strcmp(fieldnames(up.paramSet), 'sub_groups'))
    subjs = up.paramSet.subj_list;
    rel_els = find_rel_subjs(up, 'sub_group');
    sub_groups = fieldnames(rel_els);
    for group_no = 1 : length(sub_groups)
        eval([sub_groups{group_no} '_log = false(length(subjs),1);']);
        eval([sub_groups{group_no} '_log(rel_els.' sub_groups{group_no} ') = true;']);
    end
end

%% Cycle through each algorithm, subject and window
ecg_log = logical(~cellfun(@isempty, strfind(alg_names.sigs, 'ekg')));        % is it ecg or ppg
sig_types = unique(alg_names.sigs); sig_types = sig_types(:)';
for sig_type = sig_types
    eval([sig_type{1,1} '_log = strcmp(alg_names.sigs, ''' sig_type{1,1} ''');']);
    eval(['data.' sig_type{1,1} '_log = deal(single([]));']);
end
...
if sum(strcmp(fieldnames(up.paramSet), 'sub_groups'))
    for group_no = 1 : length(sub_groups)
        eval(['data.' sub_groups{group_no} '_log = single([]);']);
    end
end
for comp_no = 1 : length(comp_str)
...
for subj = subjs
    ...
    % Load CCs
    loadpath = [up.paths.data_save_folder, num2str(subj), up.paths.filenames.cc, '.mat'];
    cc_log = logical(exist(loadpath, 'file'));
    if cc_log
        CCs = load(loadpath);
        temp = fieldnames(CCs);
        eval(['CC_types = fieldnames(CCs.' temp{1,1} ');']);
        CC_types = CC_types(~strcmp(CC_types, 't'));
    end
    ...
    for s = 1 : length(alg_names.old_names)
        ...
        data.alg_no(start_el:counter_no,1) = s;
        if cc_log
            % if this is not a fusion then insert CC types:
            if isnan(alg_names.meths.fm(s)) && isnan(alg_names.meths.ft(s))
                for cc_type_no = 1 : length(CC_types)
                    rel_el = []; est_no =1;
                    while isempty(rel_el)
                        rel_el = strfind(alg_names.old_names{s}, up.al.options.estimate_rr{est_no});
                        est_no = est_no+1;
                    end
                    rel_ext_sig_name = alg_names.old_names{s}(1:rel_el-2);
                    eval(['data.' CC_types{cc_type_no} '(start_el:counter_no,1) = CCs.' rel_ext_sig_name '.' CC_types{cc_type_no} ';']);
                end
            else
                for cc_type_no = 1 : length(CC_types)
                    temp = nan(length(start_el:counter_no),1);
                    eval(['data.' CC_types{cc_type_no} '(start_el:counter_no,1) = temp;']);
                    clear temp
                end
            end
        end
        temp = strfind(alg_names.old_names{s}, '_');
        sig_type = alg_names.old_names{s}(1:temp(1)-1);
        if strcmp(sig_type, 'scg')
            sig_type = 'ekg';
        end
        eval(['data.sqi(start_el:counter_no,1) = sqi.' sig_type '.v;']);
        eval(['data.hr(start_el:counter_no,1) = sqi.' sig_type '.hr;']);
        eval(['data.hr_rr(start_el:counter_no,1) = sqi.' sig_type '.hr./rr_ref.v;']);
    end
    % variables which are already in the order of the windows, and need repeating for all the different algorithms
    data.ref = [data.ref; repmat(rr_ref.v(:), [length(alg_names.old_names),1])];
    data.snr_log = [data.snr_log; repmat(rr_ref.snr_log(:), [length(alg_names.old_names),1])];
    win_nos = 1 : length(rr_ref.v);
    data.win_no = [data.win_no; repmat(win_nos(:), [length(alg_names.old_names),1])];
    % variables which are in the order of the algorithms. Each one needs to be repeated for each window, and then move onto the next alg and repeat that for each window ...
    temp = repmat(alg_names.names(:)', [length(rr_ref.v), 1]);
    temp = temp(1:numel(temp)); temp = temp(:);
    data.alg_names = [data.alg_names; temp];
    temp = repmat(ecg_log(:)', [length(rr_ref.v), 1]);
    ...
    data.ecg_log = [data.ecg_log; temp];
    for sig_type = sig_types
        eval(['temp = repmat(' sig_type{1,1} '_log(:)'', [length(rr_ref.v), 1]);'])
        temp = temp(1:numel(temp)); temp = temp(:);
        eval(['data.' sig_type{1,1} '_log = [data.' sig_type{1,1} '_log; temp];']);
    end
    for comp_no = 1 : length(comp_str)
    ...
    if sum(strcmp(fieldnames(up.paramSet), 'sub_groups'))
        for group_no = 1 : length(sub_groups)
            eval(['data.' sub_groups{group_no} '_log = [data.' sub_groups{group_no} '_log; ' sub_groups{group_no} '_log(subj)*ones(length(rr_ref.v)*length(alg_names.old_names),1)];']);
        end
    end
end
clear win_nos comp_no s start_el counter_no est_RRs temp rr_ref loadpath subj ecg_log rel_data ppg_hr_rr_ratio sqi ekg_hr_rr_ratio
...
if sum(strcmp(fieldnames(up.paramSet), 'sub_groups'))
    for group_no = 1 : length(sub_groups)
        eval(['data.' sub_groups{group_no} '_log = logical(data.' sub_groups{group_no} '_log);']);
    end
end
data.ecg_log = logical(data.ecg_log);
data.sqi = logical(data.sqi);
...
for sig_type = sig_types
    eval(['data.' sig_type{1,1} '_log = logical(data.' sig_type{1,1} '_log);']);
end
...
[hr,rr] = deal(nan(length(data),1));
for s = 1 : length(data)
    hr(s) = data(s).ref.params.hr.v;
    rr(s) = data(s).ref.params.rr.v;
end
clear s
...
subj_rel_els = cell(length(subjs),1);
...
synth_results.ef_5_bad_algs = find( all_alg_perf < thresh & act_alg_names.meths.ef==exc_ef_no  & act_alg_names.meths.xb~=exc_xb_no);
synth_results.xb_10_algs = find(act_alg_names.meths.xb==exc_xb_no);
synth_results.xb_10_bad_algs = find( all_alg_perf < thresh & act_alg_names.meths.xb==exc_xb_no);
synth_results.analysed_algs = find(act_alg_names.meths.ef~=exc_ef_no & act_alg_names.meths.xb~=exc_xb_no);

% Additional results (commented out)
