...
for subj = up.paramSet.subj_list
    ...
    for sig_no = 1 : length(sigs)
        ...
        for s = 1 : length(var_names)
            if strfind(var_names{s}, [curr_sig, up.paths.filenames.feat_meas])
                rel_log(s) = 1;
            end
        end
        ...
        for rel_var_name_no = 1 : length(rel_var_names)
            for current_opt_no = 1 : length(up.al.options.RS)
                ...
                eval(['save_name = ''' curr_sig, up.paths.filenames.resampler, up.al.options.RS{current_opt_no}, rel_var_names{rel_var_name_no}(start_el:end) ''';']); clear start_el
