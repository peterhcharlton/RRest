function rr = ARM(rel_data, wins, up)
%ARM calculates the power spectral density of a signal using
% autoregressive modelling, and finds the RR, using the median spectral
% method.
%
%	            ARM(option, up)
%
...
for win_no = 1 : length(wins.t_start)
    ...
    %% Now that we have a processed PPG-waveform, i.e. the 'respiratory signal', lets apply the AR model
    % apply for each model order (2-20)
    for model_order = 2:20
        [Yy(:,model_order-1),Xx(:,model_order-1)]=pburg(data.filt.v,model_order,[],downsample_freq);
    end
    med_Yy = median(Yy, 2);
    data.freqs=Xx(:,1);
    data.power=med_Yy; clear Xx Yy
